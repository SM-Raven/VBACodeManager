...
class VBAExtractor:
    ...
    VBA_COMPONENT_TYPES = {
        ...
        100: ComponentType.EXCEL_OBJECT,  # ThisWorkbook
        120: ComponentType.EXCEL_OBJECT,  # Sheet module
        13: ComponentType.EXCEL_OBJECT,   # Document object
    }
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Extracting from: {wb.Name}")
        
        # Get all components
        all_components = extractor.extract_all_components()
        print(f"\nFound {len(all_components)} components:")
        for comp in all_components:
            print(f"  - {comp.name} ({comp.type.name}) -> {comp.get_folder()}")
        
        # Get single component
        if all_components:
            first_name = all_components[0].name
            single = extractor.extract_component_by_name(first_name)
            if single:
                print(f"\nExtracted single: {single.name}")
                print(f"Code length: {len(single.code)} chars")
